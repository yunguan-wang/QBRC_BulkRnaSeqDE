...
# index: STAR index
# gtf,thread,disambiguate,count: follow those in expression.pl
...
my ($jobs,$example,$index,$gtf,$thread,$disambiguate,$count,$n)=@ARGV;
...
while ($line=<JOB>)
{
  ...
  print SCRIPT "perl ".$path."/expression.pl ".$index.":".$items[0].",".$items[1]." ".$gtf." ".$items[2].
    " ".$thread." ".$items[3]." ".$disambiguate." ".$count."\n";
